...
import { extractAbhaNumberFromProfilePayload } from './utils/profile-account.util';
...
export class IdentityService {
  ...
  async getProfileAccount(xToken: string, gatewayToken: string): Promise<any> {
    ...
    try {
      ...
      return {
        status: 'error',
        message: 'Failed to fetch ABHA profile from ABDM gateway.',
        details: { error: msg },
      };
    }
  }
  ...
  /**
   * @description Resolve full 14-digit ABHA via GET profile/account (server-side only)
   */
  async resolveAbhaNumberFromProfileAccount(
    xToken: string,
  ): Promise<
    | { abhaNumber: string; ABHANumber: string }
    | { status: 'error'; message: string }
  > {
    if (!xToken?.trim()) {
      return { status: 'error', message: 'Profile session (X-token) is required.' };
    }
    ...
    try {
      ...
      return { status: 'error', message: 'Gateway session unavailable. Cannot load ABHA profile.' };
    }

    const profile = await this.getProfileAccount(xToken, gatewayToken);
      ...
      return {
        ...
        message:
          typeof profile.message === 'string'
            ? profile.message
            : 'Unable to load ABHA profile from account API. Sign in again and retry.',
      };
    }

    const extracted = extractAbhaNumberFromProfilePayload(
    ...
    if (!extracted) {
      return {
        status: 'error',
        message:
          'Could not read ABHANumber from profile account API. Sign out, log in again with ABHA OTP, then retry.',
      };
    }

    return { abhaNumber: extracted.digits, ABHANumber: extracted.ABHANumber };
  }
  /**
